...
void PassiveBuzzer_Control(int on)
{
    if (on)
    {
        //HAL_TIM_PWM_Start(g_HPWM_PassiveBuzzer, TIM_CHANNEL_1);
				__HAL_TIM_ENABLE(g_HPWM_PassiveBuzzer);
    }        
    else
    {
        //HAL_TIM_PWM_Stop(g_HPWM_PassiveBuzzer, TIM_CHANNEL_1);
			__HAL_TIM_MOE_DISABLE_UNCONDITIONALLY(g_HPWM_PassiveBuzzer);
    }
}
 ...
 * 输入参数： freq - 频率, duty - 占空比 , muteFlag - 是否静音
 * 输出参数： 无
 * 返 回 值： 无
 * 修改日期：      版本号     修改人	      修改内容
 * -----------------------------------------------
 * 2023/08/04	     V1.0	  韦东山	      创建
 ***********************************************************************/
void PassiveBuzzer_Set_Freq_Duty(int freq, int duty,char muteFlag)
{
		if(muteFlag == 1)
		{
			__HAL_TIM_MOE_DISABLE_UNCONDITIONALLY(g_HPWM_PassiveBuzzer);
    ...
    g_HPWM_PassiveBuzzer->Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    g_HPWM_PassiveBuzzer->Init.RepetitionCounter = 0;
    g_HPWM_PassiveBuzzer->Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
    HAL_TIM_Base_Init(g_HPWM_PassiveBuzzer);
...
void PassiveBuzzer_Test(void)
{
	...
	//LCD_Init();
	//LCD_Clear();
		uint16_t tempFreq = 600;
    while (1)
    {
				tempFreq += 50;
				PassiveBuzzer_Set_Freq_Duty(tempFreq,50,0);
