...
{
	lv_label_set_text(topLabel, "本通的测试程序");
}
...
void GUI_test(void)
{
    ...
    topLabel = lv_label_create(father);
    lv_obj_set_size(topLabel, father_width / 2, father_hight / 6);
    ...
    lv_obj_set_style_text_font(topLabel, &myFont_20, LV_PART_MAIN);
    lv_label_set_text(topLabel, "本通");
	lv_obj_t* btn = lv_btn_create(father);
	lv_obj_set_size(btn, 100, 50);
	lv_obj_align_to(btn, topLabel, LV_ALIGN_OUT_BOTTOM_MID, 0, 0);
