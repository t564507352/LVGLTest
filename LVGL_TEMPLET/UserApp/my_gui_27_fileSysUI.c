...
char *lv_suffix [] ={".txt",".avi",".png","jpeg",".jpg",".bin",".gif",".bmp",".FON",".dat",".sif",".BIN",".xbf",".ttf",".TXT",".PNG","JPEG",".JPG",".GIF",".BMP"};
...
lv_obj_t * lv_create_page(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(lv_flie.lv_page_cont, lv_obj_get_width(lv_scr_act()), lv_obj_get_height(lv_scr_act()));
    ...
    lv_obj_set_size(lv_page_obj, lv_obj_get_width(lv_scr_act()), lv_font_montserrat_14.line_height);
    ...
    if(code == LV_EVENT_CLICKED)
    {
        for (int i = 0; i <= lv_flie.list_flie_nuber ;i++)  /* 轮询列表项 */
        {
            if (obj == lv_flie.list_btn[i]) /* 判断列表项的那个按键按下 */
            {
                ...
                {
                    if (strstr(lv_flie.lv_pname,".TXT") || strstr(lv_flie.lv_pname,".txt") != NULL)                  /* 如果不是文件夹且是txt文件 */
                    {
                    ...
                    else if ((strstr(lv_flie.lv_pname,".bin") || strstr(lv_flie.lv_pname,".BIN")) != NULL)             /* 如果不是文件夹且是bin文件 */
                    {
                        lv_flie.lv_return_page = lv_create_page(lv_scr_act());    /* 创建页面 */
...
void lv_create_list(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(lv_flie.list, lv_obj_get_width(lv_scr_act()), lv_obj_get_height(lv_scr_act()) - lv_font_montserrat_14.line_height*2 - 60);  /* 设置列表的大小 */
...
void lv_page_tile(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(lv_flie.lv_page_obj, lv_obj_get_width(lv_scr_act()), lv_font_montserrat_14.line_height + 50);
...
void lv_page_back(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(lv_flie.lv_back_obj,lv_obj_get_width(lv_scr_act()),lv_font_montserrat_14.line_height+10);                              /* 设置改区域的大小 */
